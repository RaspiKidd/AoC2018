...
for i in range(1000000000):
    ...
    if score in scores:
        if len(scores[score]) > 3:
            if (i+1)%(i-sorted(scores[score])[-1]) == 1000000000%(i-sorted(scores[score])[-1]): #Handle indexing and avoid trying to find -1st. Originally just the current numbers, Changed so it works for other people
                print("Part 2: ", score)
                break
    scores[score].add(i)
